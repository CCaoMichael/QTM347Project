...
The key components of our approach include partitioning training and testing data, training each model on the training dataset, evaluating the test MSE on the test dataset, and then comparing the test MSEs from different models to find the best-predicting model with the lowest test MSE.
...
In running these three models, we found  the best-predicting model to be the gradient-boosted regression, which has the lowest MSE. Each model, with or without parameters, had almost identical parameters, however, the test MSE for the models without outliers is generally about half of those models with outliers. 

Therefore, we decided that the removal of outliers for our final analysis would improve the prediction accuracy of our models.

### PCR
We ran 5-fold cross-validation to find the optimal number of principal components to be 28.

![screenshot](/msc/PCA_MSE.png)

We also found the explained variance for each principal component: Few principal components  account for a significant portion of the variance in the data. Even the first, most significant principle component accounted for less than 10% of the variance.
...
We also found the top 5 most important features, being:
...
We used 5-fold cross-validation to find the optimal depth number of estimators, and the number of samples per split. We found the max depth to be 7, the minimum sample split to be 8, and the number of estimators to be 140.
...
## Discussion
Gradient boosting has the lowest MSE. This is expected because of gradient boosting’s ability to handle non-linear relationships and its high prediction accuracy. On the other hand, both PCR and linear regression with regularization assume linear relationships between variables, which might not address the complexity within variables. 
...
The most important features generated from linear regression and gradient boosting regression are different. It’s interesting to see that dummy variables, like room type and district location of Airbnb, have more importance in the linear regression model while continuous variables, like the total number of listings for a host, minimum number of stay nights, and longitude, have higher importance in gradient boosting regression. This might be because the two methods give different weights to different types of variables. Despite the differences, it's also noteworthy that the important features do overlap as well as the room type and location of the Airbnb both show up in as the most important features in the two models.
